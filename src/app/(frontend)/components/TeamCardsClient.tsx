            ...
            <SwiperSlide key={member?.id || index}>
              <div className="bg-white overflow-hidden pt-4">
                ...
                <div className="py-4 md:py-6">
                  {member?.name && (
                    <h3 className="text-[#000F19] font-manrope-semibold text-lg md:text-xl mb-1">{member.name}</h3>
                  )}
                  {member?.role && (
                    <p className="text-[#000F19] text-xs uppercase font-manrope-bold leading-4 tracking-wide mb-2 md:mb-4 border-b border-[#000F19] pb-3 md:pb-4">{member.role}</p>
                  )}
                  {member?.bio && (
