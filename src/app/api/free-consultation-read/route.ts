...
export async function GET() {
  try {
    ...
    const cacheKey = 'api-free-consultation-read'
    ...
    const freeConsultationSection = await payloadClient
      .findGlobal({ slug: 'free-consultation-section' })
      .catch(() => null)
    ...
    return NextResponse.json(
      { error: 'Failed to fetch free consultation data' },
      { status: 500 }
    )
  }
}
